...
- ✅ **DataFusion Integration**: Reading partitioned parquet files and reconstructing OTAP data
- ✅ **Schema Understanding**: Proper handling of OTAP's star-schema structure
- ✅ **Pipeline Integration**: Seamless integration with existing OTAP pipeline components
- ✅ **Arrow Performance**: Efficient RecordBatch processing and transformation
...
│   ├── File aging and safety margin handling
│   ├── Time-aligned window assembly (1-minute, 5-minute, etc.)
│   └── Partition discovery across time windows
├── DataFusion Query Engine  
│   ├── 4 Partitioned Table Registration (logs, log_attributes, resource_attributes, scope_attributes)
│   ├── Virtual partition columns (_part_id) for automatic partition handling
│   ├── Custom UDAF registration (weighted_reservoir_sample)
│   └── Query execution with predicate pushdown
├── Arrow Compute Optimization Layer
│   ├── Vectorized ID column transformations (UInt32 → UInt16 with normalization)
│   ├── Efficient array slicing (process 1K from 100K without full column materialization)
│   ├── View type materialization (UTF8View → UTF8, BinaryView → Binary)
│   └── Memory-efficient batch processing
└── OTAP Reconstruction Engine
    ├── Query result processing (log_attributes as primary output)
    ├── Related table filtering (logs, resource_attributes, scope_attributes)
    ├── Streaming merge coordination
    └── OtapArrowRecords generation
```

### Data Flow Architecture

```text
Parquet Files → Temporal Windows → DataFusion Query → Arrow Optimization → OTAP Output
      ↑                ↑                 ↑                    ↑                 ↑
...
Instead of registering N×4 individual tables, we use **4 partitioned ListingTables** with automatic partition column injection:
...
-- ✅ DataFusion: Automatic predicate pushdown and partition pruning
SELECT *
FROM log_attributes
...
```sql
-- ✅ Clean JOIN between partitioned tables with automatic partition alignment
SELECT la.*
...
```sql
-- ✅ Advanced sampling with custom UDAF
...
```rust
// 🐌 SLOW: Element-by-element processing with builders
...
```rust
// ✅ FAST: Vectorized operations with Arrow compute kernels
...
- **Memory Allocation**: Linear → Constant (vectorized operations)
...
- Implement streaming patterns (process → send → drop)
...
**🚀 Ready to Begin Implementation - Let's build the future of OTAP data processing!**
