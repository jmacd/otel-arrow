...
Add a fourth OTAP signal, **Flight Recorder**, represented by a fixed and
bounded set of Arrow `RecordBatch` schemas. Flight Recorder data represents
...
### Flight Recorder is an OTAP signal

Flight Recorder is a pipeline signal alongside logs, metrics, and traces:
...
Its in-memory OTAP representation is a validated set of Arrow record batches:

```rust
enum OtapArrowRecords {
    Logs(Logs),
    Metrics(Metrics),
    Traces(Traces),
...
There is initially no OTLP Flight Recorder service. `FlightRecords` is an OTAP
extension signal. Components that do not declare support for it reject it
rather than silently converting or dropping it.
...
A receiver demultiplexes these classes into their corresponding OTAP signals.
An exporter may interleave all five signals in one V6 stream while preserving
their record class and stream ordering. If both a source observation and its
derived normalized signal are recorded, the normalized record must be marked
as derived and linked to its source capture or record range so a reader does
not interpret the pair as independent duplicate observations.
...
Recorder signal. Conversion to Profiles may dictionary-encode and intern
...
Flight Recorder is a graph signal. Existing root-only batch operations are not
sufficient.
...
### Association with other OTAP signals
...
For Flight Recorder, the signal root is `FLIGHT_RECORDS`. For the existing
signals it is logs, spans, or metrics. A derived signal inherits the Flight
record's resource and scope unless a documented semantic mapping establishes a
different scope. A source provider name alone is not sufficient to invent an
...
| MetadataBlock | event schemas and schema fields |
| EventBlock | root records and event values |
| StackBlock | stack frames and locations |
| Thread and thread-removal blocks | thread context and lifecycle records |
| LabelListBlock | correlation columns and attributes |
| SequencePointBlock | sequence-point and loss records |
...
- `SignalType::FlightRecords` in `otap_df_config`;
- `OtapArrowRecords::FlightRecords` and a validated `FlightRecords` store in
  `otap_df_pdata`;
- new Arrow payload enum values in the experimental Arrow protocol;
- canonical Arrow schemas and schema IDs for all payloads;
- views and builders;
- retained-memory and logical-size accounting;
- split, concatenate, filter, sanitize, and reindex support;
- OTAP Arrow IPC encode/decode support;
- signal-aware routing and component capability declarations; and
- explicit rejection by OTLP exporters until data is transformed into a
  standard OpenTelemetry signal.

The experimental Arrow protocol change requires coordination with other OTAP
implementations. Numeric payload values must be assigned through that process;
this RFC intentionally names payloads without assigning enum numbers.
...
- A fourth signal affects exhaustive matches throughout configuration, pdata,
  routing, batching, views, exporters, tests, and the experimental Arrow
  protocol.
...
Without a Flight Recorder signal, the one_collect receiver must either emit
irreversible logs/metrics/traces, retain opaque source blobs, or create a
component-specific representation outside pdata. NetTrace V6 replay and future
Profiles integration would then duplicate decoding, stack, and symbol logic.
...
10. What numeric values and compatibility policy will the experimental Arrow
    protocol assign to the new payload types?
...
- A common mixed-signal flight-recorder format containing source records,
  logs, metrics, traces, and profiles in one time-ordered V6 stream.
