...
impl PipelineDag {
    ...
    pub fn validate(&self) -> Result<(), Vec<ExperimentalError>> {
        let mut errors = Vec::new();

        if let Some(cycle) = self.detect_cycle() {
            errors.push(ExperimentalError::CycleDetected {
                nodes: cycle.into_iter().map(Cow::Owned).collect(),
            });
        }
        ...
        for edge in &self.edges {
            let from_node = match self.nodes.get(&edge.from) {
                Some(n) => n,
                None => {
                    errors.push(ExperimentalError::UnknownNode(Cow::Owned(
                        edge.from.clone(),
                    )));
                    continue;
                }
            ...
            let to_node = match self.nodes.get(&edge.to) {
                Some(n) => n,
                None => {
                    errors.push(ExperimentalError::UnknownNode(Cow::Owned(edge.to.clone())));
                    continue;
                }
            };
            if from_node.output_type != to_node.input_type {
                errors.push(ExperimentalError::TypeMismatch {
    ...
    fn topological_sort(&self) -> Result<Vec<String>, ExperimentalError> {
        ...
        while let Some(n) = queue.pop_front() {
            ...
            for e in self.edges.iter().filter(|e| e.from == n) {
                let d = in_degree
                    .get_mut(&e.to)
                    .ok_or(ExperimentalError::UnknownNode(Cow::Owned(e.to.clone())))?;
                *d -= 1;
                if *d == 0 {
        ...
        if sorted.len() == self.nodes.len() {
            ...
            Err(ExperimentalError::CycleDetected {
                nodes: vec!["Cycle in topological sort".into()],
            })
        }
    }
}
...
#[cfg(test)]
mod tests {
    use crate::experimental::{PipelineDag, SignalType};
    use crate::node::NodeKind;
