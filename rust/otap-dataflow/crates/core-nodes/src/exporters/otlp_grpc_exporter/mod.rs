...
mod tests {
    ...
    fn test_receiver_not_ready_on_start_and_reconnect() {
        ...
        async fn drive_test(
            ...
            mut pipeline_completion_msg_rx: otel_arrow_dfe_engine::control::PipelineCompletionMsgReceiver<
                OtapPdata,
            >,
    ...
    #[tonic::async_trait]
    impl otel_arrow_dfe_pdata::proto::opentelemetry::collector::logs::v1::logs_service_server::LogsService
    ...
    fn documented_composite_context_reaches_grpc_metadata() {
        ...
        use otel_arrow_dfe_engine::context_declaration::CompiledContextAccess;
        ...
        let capture = compiled
            .node_bindings(&pipeline, &"receive".into())
            .unwrap()
            .iter()
        ...
        let propagation = compiled
            .node_bindings(&pipeline, &"export".into())
            .unwrap()
            .iter()
            .find_map(|binding| match &binding.access {
                CompiledContextAccess::HeaderPropagation(sink) => Some(sink.as_ref().clone()),
                _ => None,
            })
            .unwrap();
