...
use crate::tokenbucket::{Clock, Limit, Limiter, SystemClock};
use crate::Error;
...
pub struct RateLimitProcessor<PData> {
    ...
    pub limiter: Limiter<SystemClock>,
...
impl<PData> RateLimitProcessor<PData> {
    ...
    pub fn new(config: RateLimitConfig) -> Result<Self, Error> {
        ...
        let limiter = Limiter::new(limit, config.burst, SystemClock)?;
    ...
    async fn process(
        ...
        match msg {
            Message::PData(data) => {
                ...
                match self.limiter.reserve_n(1) {
                    Ok(reservation) => {
                        ...
                        if reservation.delay_from(SystemClock.now()).is_zero() {
            ...
            Message::Control(control_msg) => {
                ...
                match control_msg {
                    NodeControlMsg::Shutdown { deadline: _, reason: _ } => {
...
mod tests {
    ...
    fn test_rate_limit_processor_creation() {
        ...
        assert_eq!(processor.limiter.tokens_at(SystemClock.now()), 5.0);
    }
