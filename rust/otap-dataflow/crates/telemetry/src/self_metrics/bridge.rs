...
mod tests {
    ...
    fn view_renames_metric() {
        ...
        // 3 non-Mmsc metrics, same count as without views.
        assert_eq!(ds.number_field_indices.len(), 3);

        // Verify data points builder works with view-renamed schema.
        let dp = ds
            .schema
            .data_points_builder()
            .build_int_values(1_000_000_000, 2_000_000_000, &[10, 5, 3])
            .unwrap();
        assert_eq!(dp.num_rows(), 3);
    }
    ...
    fn view_wildcard_scope_matches_any() {
        ...
        // 3 metrics (all non-Mmsc), same count as without views.
        assert_eq!(ds.schema.metrics_batch().num_rows(), 3);

        // Build data points to verify the builder still works with views.
        let dp = ds
            .schema
            .data_points_builder()
            .build_int_values(1_000_000_000, 2_000_000_000, &[10, 5, 3])
