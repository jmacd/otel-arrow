...
use crate::{
    otap::{
        error::{self, Result},
        Logs, Metrics, OtapArrowRecordTag, OtapArrowRecords, OtapBatchStore, Traces,
        POSITION_LOOKUP,
    },
...
impl RecordsGroup {
    ...
    pub fn rebatch(self, max_output_batch: Option<NonZeroU64>) -> Result<Self> {
        let effective_max = max_output_batch
            .unwrap_or_else(|| NonZeroU64::new(u16::MAX as u64).unwrap());
...
fn rebatch_logs_single_pass<const N: usize>(
    ...
    _max_batch_size: NonZeroU64,
) -> Result<Vec<[Option<RecordBatch>; N]>> {
    // TODO: Implement piece-by-piece rebatching
